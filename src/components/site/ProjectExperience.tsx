...
function directoryProjectFor(input: {
  ...
  const candidates = GURGAON_DIRECTORY_PROJECTS.filter((item) => {
    const aliases = [item.name, ...item.inventoryAliases]
      .map(normalize)
  ...
  return (
    candidates.find((item) => normalize(item.sector) === normalizedSector) ?? candidates[0] ?? null
  );
}
...
export function ProjectExperience({
  ...
  const projectName =
    intelligence?.name ?? directoryProject?.name ?? project?.name ?? identity?.name ?? "Project";
  const projectHref = identity
    ? projectApartmentPath(identity.slug)
    : project?.slug
      ? projectApartmentPath(project.slug)
      : null;
  ...
  const officialSources =
    intelligence?.officialSourceUrls ??
    (directoryProject?.officialSourceUrl ? [directoryProject.officialSourceUrl] : []);
  ...
  const fallbackDescription = directoryProject
    ...
    : null;
  const description =
    intelligence?.projectDescription ?? project?.description ?? fallbackDescription;
  ...
  return (
    <div className="space-y-10">
      <section aria-labelledby="about-project">
        ...
        {description ? (
          <p className="mt-4 max-w-4xl leading-7 text-muted-foreground">{description}</p>
        ) : null}
        <dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {intelligence?.landArea ? (
            <Fact label="Project scale" value={intelligence.landArea} />
          ) : null}
          {intelligence?.towerFloorSummary ? (
            <Fact label="Towers / floors" value={intelligence.towerFloorSummary} />
          ) : null}
      ...
      {intelligence?.amenities.length ? (
        <section aria-labelledby="project-amenities">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gold">
            Verified resident facilities
          </p>
          <h2 id="project-amenities" className="mt-2 font-display text-3xl">
            Project amenities
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
            The facilities below are shown as crawlable text and are limited to amenities
            established from the cited project sources. Unit-specific features are shown separately.
          </p>
      ...
      {intelligence?.highlights.length ? (
        <section aria-labelledby="project-highlights">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gold">
            Why buyers shortlist the society
          </p>
          <h2 id="project-highlights" className="mt-2 font-display text-3xl">
            Project highlights
          </h2>
      ...
      {corridor ? (
        <section aria-labelledby="project-connectivity">
          ...
          <h2 id="project-connectivity" className="mt-2 font-display text-3xl">
          ...
          <div className="mt-5 grid gap-5 md:grid-cols-2">
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm leading-6 text-muted-foreground">
                {(intelligence?.roadConnectivity ?? [corridor]).map((item) => (
                  <li key={item}>• {item}</li>
                ))}
              </ul>
              <Link
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm leading-6 text-muted-foreground">
                {[
                  ...(intelligence?.businessHubsNearby ?? []),
                  ...(intelligence?.hospitalsNearby ?? []),
                  ...(intelligence?.schoolsNearby ?? []),
                  ...(intelligence?.shoppingNearby ?? []),
                ]
                  .slice(0, 8)
                  .map((item) => (
                    <li key={item}>• {item}</li>
                  ))}
                {!intelligence?.businessHubsNearby?.length &&
                !intelligence?.hospitalsNearby?.length &&
                !intelligence?.schoolsNearby?.length &&
                !intelligence?.shoppingNearby?.length ? (
                  <li>
                    Sector-specific school, hospital, shopping and business-hub details are added
                    only after source review rather than inferred automatically.
                  </li>
                ) : null}
              </ul>
            </div>
      ...
      {officialSources.length ? (
        <section className="rounded-xl border border-border bg-muted/30 p-5 text-sm text-muted-foreground">
          ...
          <p className="mt-1 leading-6">
            ...
            Current availability, asking price and unit-specific details are confirmed at the time
            of enquiry.
          </p>
