...
const PROJECT_VIDEOS: Record<string, readonly VideoPreview[]> = {
  ...
  "/dlf-skycourt-sector-86-gurgaon-apartments": [
    {
  ...
  return (
    <>
      <section
        ...
        aria-labelledby="project-video-previews-title"
      >
        <div className="container-page py-14 md:py-16">
          <div className="max-w-3xl">
            ...
            <h2
              id="project-video-previews-title"
              className="mt-3 font-display text-3xl md:text-4xl"
            >
              Walkthrough previews
            </h2>
            <p className="mt-4 leading-7 text-muted-foreground">
              These are lightweight web previews prepared from Shubh Estate Brokers' original
              property walkthroughs. Contact us for the full walkthrough and current unit
              availability.
            </p>
          </div>

          <div
            className={`mt-8 grid gap-6 ${videos.length > 1 ? "md:grid-cols-2 xl:grid-cols-3" : "max-w-2xl"}`}
          >
            {videos.map((video) => (
              <figure
                key={video.title}
