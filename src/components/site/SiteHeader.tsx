          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="w-80 overflow-y-auto">
              ...
              <nav className="mt-6 grid gap-1" aria-label="Mobile">
                ...
                <MobileGroup
                  title="Properties"
                  primaryTo="/flats-for-sale-in-gurgaon"
                  items={PROPERTY_LINKS}
                  onClose={() => setOpen(false)}
                />
                <MobileGroup
                  title="Owner Services"
                  primaryTo="/sell-property-gurgaon"
                  items={OWNER_LINKS}
                  onClose={() => setOpen(false)}
                />
...
function NavDropdown({
  ...
  label: string;
  to: string;
  items: readonly { to: string; label: string; description: string }[];
}) {
  return (
