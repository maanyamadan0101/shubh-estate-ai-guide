...
export function PropertyView({
  ...
  const gallery = images.length
    ...
    : property.cover_image_url
      ? [{ id: "cover", image_url: property.cover_image_url, alt_text: property.title, is_primary: true }]
      : [];
  ...
  const specs = [
    ...
    property.carpet_area_sqft ? { icon: Ruler, label: "Carpet area", value: formatArea(property.carpet_area_sqft) } : null,
    property.bathrooms ? { icon: Building2, label: "Bathrooms", value: String(property.bathrooms) } : null,
    property.balconies ? { icon: Building2, label: "Balconies", value: String(property.balconies) } : null,
    property.floor_number
      ? { icon: Layers, label: "Floor", value: `${property.floor_number}${property.total_floors ? ` of ${property.total_floors}` : ""}` }
      : null,
    ...
    property.parking ? { icon: Car, label: "Parking", value: `${property.parking} space${property.parking > 1 ? "s" : ""}` } : null,
    ...
    { icon: ShieldCheck, label: "Possession", value: STATUS_LABEL[property.status] ?? property.status },
  ...
  return (
    <>
      {isPreview ? (
        <div className="bg-gold/15 py-2 text-center text-xs uppercase tracking-[0.2em] text-foreground">Preview — not visible to the public</div>
      ) : null}
        ...
        <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="bg-gold text-gold-foreground hover:bg-gold">{STATUS_LABEL[property.status]}</Badge>
      ...
      {gallery.length ? (
        <section className="container-page mt-6">
          <div className="overflow-hidden rounded-xl border border-border bg-muted">
            <img src={gallery[active]!.image_url} alt={gallery[active]!.alt_text ?? property.title} className="aspect-[16/10] w-full object-cover" width={1600} height={1000} />
          </div>
          {gallery.length > 1 ? (
            <ul className="mt-3 flex gap-3 overflow-x-auto pb-2">
              {gallery.map((image, i) => (
                <li key={image.id}>
                  <button type="button" onClick={() => setActive(i)} aria-label={`Show photo ${i + 1}`} className={`overflow-hidden rounded-md border-2 transition-colors ${i === active ? "border-gold" : "border-transparent"}`}>
                    <img src={image.image_url} alt={image.alt_text ?? ""} loading="lazy" className="size-20 object-cover" />
                  </button>
                </li>
              ))}
            </ul>
        ...
        <section className="container-page mt-8">
          ...
          <div className="mt-4 grid gap-5 md:grid-cols-2">
            {videos.map((url, index) => {
              ...
              return (
                <div key={`${url}-${index}`} className="overflow-hidden rounded-xl border border-border bg-black">
                  {embed ? (
                    <iframe
        ...
        <div className="space-y-10">
          ...
          {property.description ? (
            <div>
              ...
              <div className="mt-4 space-y-4 text-muted-foreground">
                {String(property.description).split(/\n{1,}/).filter((p: string) => p.trim()).map((paragraph: string, i: number) => <p key={i}>{paragraph}</p>)}
              </div>
            </div>
          ) : null}
          ...
          {features.length ? (
            <div>
              ...
              <ul className="mt-4 flex flex-wrap gap-2">
                {features.map((f) => <li key={f}><Badge variant="secondary" className="font-normal">{f}</Badge></li>)}
              </ul>
            </div>
          ) : null}
              ...
              <ul className="mt-4 grid gap-2 sm:grid-cols-2 md:grid-cols-3">
                {amenities.map((a) => (
                  <li key={a} className="flex items-center gap-2 text-sm text-muted-foreground">
                    <ShieldCheck className="size-4 text-gold" aria-hidden="true" />{a}
                  </li>
                ))}
              </ul>
            </div>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                {property.project ? (
                  <div className="rounded-lg border border-border bg-card p-5">
                    ...
                    {property.project.description ? <p className="mt-2 text-sm text-muted-foreground">{property.project.description}</p> : null}
                    {property.project.rera_number ? <p className="mt-2 text-xs text-muted-foreground">RERA: {property.project.rera_number}</p> : null}
                  </div>
                ) : null}
                {property.builder ? (
                  <div className="rounded-lg border border-border bg-card p-5">
                    <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Developer</p>
                    <p className="mt-1 font-display text-xl">{property.builder.name}</p>
                    {property.builder.description ? <p className="mt-2 text-sm text-muted-foreground">{property.builder.description}</p> : null}
                  </div>
                ) : null}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-border">
              <iframe title={`Map of ${property.title}`} src={`https://www.google.com/maps?q=${mapQuery}&output=embed`} loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="h-80 w-full border-0" />
            </div>
          </div>
        </div>
      ...
      {related.length ? (
        <section className="container-page pb-16">
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {related.slice(0, 3).map((item) => <ListingCard key={item.id} property={item} />)}
          </div>
        </section>
      ) : null}
    </>
