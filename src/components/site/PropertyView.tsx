...
export function PropertyView({
  ...
  const { property, images, amenities, features } = data;
          ...
          {features.length ? (
            <div>
              ...
              <ul className="mt-4 flex flex-wrap gap-2">
                {features.map((f) => (
                  <li key={f}>
                    <Badge variant="secondary" className="font-normal">{f}</Badge>
                  </li>
                ))}
              </ul>
            </div>
                ...
                {property.project ? (
                  <div className="rounded-lg border border-border bg-card p-5">
                    ...
                    {property.project.description ? (
                      <p className="mt-2 text-sm text-muted-foreground">{property.project.description}</p>
                    ) : null}
                    {property.project.rera_number ? (
                      <p className="mt-2 text-xs text-muted-foreground">RERA: {property.project.rera_number}</p>
                    ) : null}
                  </div>
                ) : null}
                {property.builder ? (
                  <div className="rounded-lg border border-border bg-card p-5">
                    ...
                    {property.builder.description ? (
                      <p className="mt-2 text-sm text-muted-foreground">{property.builder.description}</p>
                    ) : null}
                  </div>
                ) : null}
              </div>
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <div className="mt-5 grid gap-2">
              ...
              <Button asChild variant="goldOutline">
                <a href={`${CONTACT.whatsapp}?text=${whatsappText}`} target="_blank" rel="noreferrer">
            ...
            <div className="mt-6 border-t border-border pt-5">
              ...
              <div className="mt-4">
                <EnquiryForm propertyId={property.id} interest={`Private viewing — ${property.title}`} compact />
              </div>
            </div>
          </div>
        </aside>
