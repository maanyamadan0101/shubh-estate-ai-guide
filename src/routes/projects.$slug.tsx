...
export const Route = createFileRoute("/projects/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const rentCount = loaderData.listings.filter(
      (listing) => listing.listing_type === "rent",
    ).length;
    ...
    const priceContext = hasAskingPrices
    ...
    <>
        ...
        <div className="container-page py-12 md:py-16">
          ...
          <div className="mt-6 max-w-4xl">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">
              Gurgaon Project & Inventory Guide
            </p>
            <h1 className="mt-3 font-display text-4xl leading-tight md:text-5xl">
              {hub.name}
              {hub.sector ? `, ${hub.sector} Gurgaon` : " Gurgaon"}
            </h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground">
              Compare the project with the actual units currently published by Shubh Estate Brokers.
              Asking prices, floor, facing, area and availability are unit-specific and should be
              reconfirmed before a visit or token payment.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Button asChild variant="gold">
                <a href="#current-listings">View Current Listings</a>
              </Button>
              <Button asChild variant="goldOutline">
                <a href="#buyer-checks">Buyer Checks</a>
              </Button>
            </div>
          </div>
        </div>
      ...
      <section className="border-b border-border bg-card">
        <div className="container-page grid gap-px py-0 sm:grid-cols-2 lg:grid-cols-4">
          {[
            [
              "Current inventory",
              `${hub.listings.length} listing${hub.listings.length === 1 ? "" : "s"}`,
            ],
            ["Asking-price context", priceRange(numericPrices)],
            ...
            <div
              key={label}
              className="border-b border-border px-5 py-5 sm:border-b-0 sm:border-r last:border-r-0"
            >
      ...
      >
        <div className="container-page flex gap-5 overflow-x-auto py-3 text-sm">
          ...
          <a href="#current-listings" className="whitespace-nowrap hover:text-gold">
            Current listings
          </a>
          <a href="#price-size" className="whitespace-nowrap hover:text-gold">
            Price & sizes
          </a>
          <a href="#buyer-checks" className="whitespace-nowrap hover:text-gold">
            Buyer checks
          </a>
        ...
        <main className="space-y-12">
          <section id="overview" className="scroll-mt-36">
            ...
            {hub.description ? (
              ...
              <p className="mt-4 leading-7 text-muted-foreground">
                This page is built around currently published Shubh Estate Brokers inventory rather
                than a copied brochure. Use it to compare available configurations, asking prices,
                sizes, possession status and unit-level differences before shortlisting a property.
              </p>
            )}
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <InfoCard
                ...
                value={configurations.join(", ") || "See inventory"}
              />
              <InfoCard
                icon={ShieldCheck}
                label="Possession mix"
                value={
                  statuses.map((status) => STATUS_LABEL[status!] ?? status).join(", ") ||
                  "Confirm per unit"
                }
              />
              <InfoCard
                icon={MapPin}
                label="Micro-market"
                value={[hub.sector, hub.locality].filter(Boolean).join(" · ") || "Gurgaon"}
              />
            </div>
            {hub.builder_name || hub.rera_number ? (
              <div className="mt-6 rounded-xl border border-border bg-card p-5 text-sm text-muted-foreground">
                {hub.builder_name ? (
                  <p>
                    <strong className="text-foreground">Developer:</strong> {hub.builder_name}
                  </p>
                ) : null}
                {hub.rera_number ? (
                  <p className="mt-1">
                    <strong className="text-foreground">RERA reference:</strong> {hub.rera_number}
                  </p>
                ) : null}
              </div>
            ) : null}
          ...
          <section id="current-listings" className="scroll-mt-36">
            ...
            <h2 className="mt-2 font-display text-3xl">
              Current properties available in {hub.name}
            </h2>
            <p className="mt-3 leading-7 text-muted-foreground">
              These are the units currently represented in our published catalogue. Multiple
              apartments in the same project remain separate because floor, facing, view, condition,
              ownership and asking price can materially change the decision.
            </p>
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-4 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                      {listing.bhk ? (
                        <p>
                          <strong className="text-foreground">Config:</strong> {listing.bhk}
                        </p>
                      ) : null}
                      {listing.area_sqft ? (
                        <p>
                      ...
                      {listing.floor ? (
                        <p>
                          <strong className="text-foreground">Floor:</strong> {listing.floor}
                        </p>
                      ) : null}
                      {listing.facing ? (
                        <p>
                          <strong className="text-foreground">Facing:</strong> {listing.facing}
                        </p>
                      ) : null}
                    </div>
          ...
          <section id="price-size" className="scroll-mt-36">
            ...
            <h2 className="mt-2 font-display text-3xl">
              Compare the exact unit, not only the project name
            </h2>
            <p className="mt-4 leading-7 text-muted-foreground">
              The published inventory currently spans {areaText} with asking-price context of{" "}
              {priceRange(numericPrices)}. This is not a project-wide valuation. A fair comparison
              should adjust for tower, floor, facing, view, condition, parking, furnishing, dues,
              transfer terms and the seller's timeline.
            </p>
          </section>

          <section id="buyer-checks" className="scroll-mt-36">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gold">
              Resale & purchase checks
            </p>
          ...
          <section id="location" className="scroll-mt-36">
            ...
            <p className="mt-4 leading-7 text-muted-foreground">
              Evaluate the actual approach roads, commute pattern, occupied surroundings, schools,
              healthcare and daily-use conveniences relevant to your household. Travel-time claims
              can vary materially by time of day and route.
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              <Button asChild variant="goldOutline">
                <a href={corridor}>Explore corridor guide</a>
              </Button>
              <Button asChild variant="goldOutline">
                <Link to="/flats-for-sale-in-gurgaon">Browse all properties</Link>
              </Button>
            </div>
          </section>
          ...
          <section id="faq" className="scroll-mt-36">
            ...
            <div className="mt-6 space-y-4">
              <Faq
                q={`How many ${hub.name} properties are currently published here?`}
                a={`This project hub currently shows ${hub.listings.length} published listing${hub.listings.length === 1 ? "" : "s"}. Availability can change, so confirm the exact unit before planning a visit.`}
              />
              <Faq
                q={`What is the current asking-price range in ${hub.name}?`}
                a={`The currently published inventory shows ${priceRange(numericPrices)}. Treat this as asking-price context, not a guaranteed transaction value or project-wide valuation.`}
              />
              <Faq
                q={`Can Shubh Estate Brokers compare different units in ${hub.name}?`}
                a="Yes. We compare unit-level factors such as tower, floor, facing, view, condition, parking, price, documentation and financing rather than assuming every apartment in the project is equivalent."
              />
              <Faq
                q="Can a home loan be coordinated for these listings?"
                a="Home-loan assistance can be coordinated for eligible buyers and qualifying properties. Final eligibility, valuation, documentation acceptance and sanction remain subject to the lending institution's policy."
              />
            </div>
          </section>
        </main>
        ...
        <aside className="lg:sticky lg:top-32 lg:self-start">
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Share your budget, preferred configuration and whether the purchase is for end use or
              investment.
            </p>
            <div className="mt-5">
              <EnquiryForm interest={`${hub.name} current listings`} compact />
            </div>
